...
class TrainingAPIClient:
    ...
    def start_training(
        self, agent_name: str, dataset_type: str = "open_images", epochs: int = 3
    ) -> Dict[str, Any]:
    ...
    def create_pipeline(self, agent_name: str, dataset_type: str = "open_images") -> Dict[str, Any]:
        ...
        """
        response = requests.post(
            f"{self.training_url}/pipeline/{agent_name}", params={"dataset_type": dataset_type}
        )
        response.raise_for_status()
        return response.json()
